...
petit point sur les methodes "virtual" dans Neurone.h :
  ...
  ce sera la methode de Neurone qui sera appellé, car pour le compilateur, on a un objet Neurone
  En mentionant que la methode de l'ojet pere est virtuelle, on dit au compilateur
  "Hey poto, je risque d'appeller cette methode depuis un objet qui semble etre un
  ...
  On est dans Layer, on cree un pack de Neurone, a la creation, on choisit si ce sont des Neurone, ou des NeuroneB
  on les stocke quoi qu'il arrive dans un vector<Neurone*> et on pourra ensuite travailler de la meme maniere, qu'il
  s'agisse de NeuroneB ou non.*/
class NeuroneB : public Neurone {
public:
  NeuroneB(int taille); //poids et biais random
